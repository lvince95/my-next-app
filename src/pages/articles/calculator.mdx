...
The regex we use, `^[\d\s+\-*/().]+$`, is broken down as follows:

- ^ and $ assert the start and end of the string, respectively, to ensure the entire string matches the pattern
- \d matches any digit (0-9)
- \s matches any whitespace character (including space, tab, newline, etc.)
- `+`, `-`, `*`, `/`, `(`, `)`, `.` are matched literally as they are commonly used math symbols.
